...
class UnifiedInventoryGenerator:
    ...
    def detect_python_interpreter(self, config: Dict, discovered_ips: Dict[str, str]) -> Optional[str]:
        """Detect Python interpreter path on target nodes"""
        ansible_config = config.get('ansible', {})
        configured_path = ansible_config.get('python_interpreter', '')
        
        # If explicitly set and not empty, validate it exists on nodes first
        if configured_path and configured_path.strip():
            configured_path = configured_path.strip()
            self.print_info(f"Validating configured Python interpreter: {configured_path}")
            
            # Validate the configured path exists on a sample node
            if self._validate_python_path(configured_path, config):
                self.print_success(f"Python interpreter validated: {configured_path}")
                return configured_path
            else:
                self.print_warning(f"Configured Python path '{configured_path}' not found on nodes, attempting auto-detection...")
                # Fall through to auto-detection
        
        # Try to detect from a sample node
        nodes_config = config.get('nodes', {})
        sample_node_ip = None
        sample_node_name = None
        
        # Try to get a master node first, then worker, then haproxy
        for master in nodes_config.get('masters', []):
            if isinstance(master, dict) and 'ip' in master:
                sample_node_ip = master.get('ip')
                sample_node_name = master.get('hostname', 'master')
                break
        
        if not sample_node_ip:
            for worker in nodes_config.get('workers', []):
                if isinstance(worker, dict) and 'ip' in worker:
                    sample_node_ip = worker.get('ip')
                    sample_node_name = worker.get('hostname', 'worker')
                    break
        
        if not sample_node_ip:
            for haproxy in nodes_config.get('haproxy', []):
                if isinstance(haproxy, dict) and 'ip' in haproxy:
                    sample_node_ip = haproxy.get('ip')
                    sample_node_name = haproxy.get('hostname', 'haproxy')
                    break
        
        if sample_node_ip:
            self.print_step(f"Detecting Python interpreter on {sample_node_name} ({sample_node_ip})...")
            
            # Get SSH user and key from config
            ssh_user = ansible_config.get('user', 'root')
            ssh_key = ansible_config.get('ssh_private_key_file', '~/.ssh/id_rsa')
            ssh_key = os.path.expanduser(ssh_key)
            
            # Try to detect Python via SSH
            common_paths = [
                '/usr/bin/python3',
                '/usr/bin/python3.9',
                '/usr/bin/python3.8',
                '/usr/bin/python3.11',
                '/usr/bin/python3.10',
                '/usr/libexec/platform-python',  # RHEL 8+
                '/opt/bin/python',  # Flatcar
                'python3',  # In PATH
            ]
            ...
            for python_path in common_paths:
                try:
                    ...
                    if python_path.startswith('/'):
                        ...
                        cmd = f"test -x {python_path} && {python_path} --version 2>&1 | head -1"
                    else:
                        # Command in PATH
                        cmd = f"which {python_path} >/dev/null 2>&1 && {python_path} --version 2>&1 | head -1"
                    ...
                    ssh_cmd.extend([f'{ssh_user}@{sample_node_ip}', cmd])
                    
                    result = subprocess.run(ssh_cmd, capture_output=True, text=True, timeout=10)
                    
                    if result.returncode == 0 and result.stdout.strip():
                        detected_path = python_path
                        # If we used 'python3', get the actual path
                        if not python_path.startswith('/'):
                            which_cmd = ['ssh', '-o', 'StrictHostKeyChecking=no',
                                        '-o', 'UserKnownHostsFile=/dev/null',
                                        '-o', 'ConnectTimeout=5']
                            if ssh_key and os.path.exists(ssh_key):
                                which_cmd.extend(['-i', ssh_key])
                            which_cmd.extend([f'{ssh_user}@{sample_node_ip}', 'which python3'])
                            which_result = subprocess.run(which_cmd, capture_output=True, text=True, timeout=5)
                            if which_result.returncode == 0:
                                detected_path = which_result.stdout.strip()
                        
                        self.print_success(f"Detected Python interpreter: {detected_path}")
            ...
            self.print_warning(f"Could not detect Python on {sample_node_name}, will use auto-discovery")
        else:
            self.print_warning("No sample node available for Python detection, will use auto-discovery")
        
        # Return None to use auto-discovery
        return None
    
    def _validate_python_path(self, python_path: str, config: Dict) -> bool:
        """Validate that Python path exists and is executable on a sample node"""
        nodes_config = config.get('nodes', {})
        ansible_config = config.get('ansible', {})
        ...
        if not sample_node_ip:
            # No nodes available for validation, assume path is valid
            return True
        ...
        try:
            # Check if Python path exists and is executable
            if python_path.startswith('/'):
                # Absolute path - check if exists and is executable
                cmd = f"test -x {python_path} && {python_path} --version >/dev/null 2>&1"
            else:
                # Command in PATH
                cmd = f"which {python_path} >/dev/null 2>&1 && {python_path} --version >/dev/null 2>&1"
            ...
            # Timeout - assume path might be valid but can't verify
            return True
        except Exception:
            # Any error - assume path might be valid but can't verify
            return True
    ...
    """
        ...
        # Detect or get Python interpreter
        python_interpreter = self.detect_python_interpreter(config, discovered_ips)
        
        # Build [all:vars] section
        all_vars_section = """\n[calico_rr]
    ...
    """
        ...
        # Add Python interpreter if detected or specified
        if python_interpreter and python_interpreter.strip():
            all_vars_section = all_vars_section.replace(
                '[all:vars]',
                f'[all:vars]\n    # Python interpreter (auto-detected or configured)\n    ansible_python_interpreter={python_interpreter}'
            )
        else:
            # If not detected, add comment about auto-discovery
            all_vars_section = all_vars_section.replace(
                '[all:vars]',
                '[all:vars]\n    # Python interpreter: auto-discovery (Ansible will detect automatically)'
            )
        
        content += all_vars_section
